...
class Intro(Scene):
    def construct(self):
        ...
        statement_tex = Tex(r"We know a concrete asymptotically optimal algorithm for factoring. ").shift(3*UP)
        downarrow_tex = Tex(r"$\Downarrow ?$").scale(2).next_to(statement_tex, DOWN, buff = 1)  
        turing_img = ImageMobject("img/award.jpg").scale(2).next_to(downarrow_tex, DOWN, buff = 1)
        ...
        statement2_tex = Tex(r"1) We don't know it's time complexity!").next_to(statement_tex, DOWN, buff = 1)
        ...
        statement3_tex = Tex(r"2) It is insanely slow. ").next_to(statement2_tex, DOWN, buff = 0.5).align_to(statement2_tex, LEFT)
        self.play(
            FadeIn(statement3_tex)
        )
        self.wait()
        ...
        select_tex = Tex("{{Asymptotic time complexity of select sort }}{{$= O(n^2)$. }}").next_to(statement3_tex, DOWN, buff = 1).to_edge(LEFT)
        select_tex2 = Tex("{{Actual time complexity: }}{{$= n^2/2 + 3n + 10$. }}").next_to(select_tex, DOWN, buff = 0.5).align_to(select_tex, LEFT)
        select_tex2[1].next_to(select_tex[1], DOWN, buff = 0.5).align_to(select_tex[1], LEFT)
        ...
        self.play(
            FadeOut(Group(
                select_tex[0], select_tex[1][0], select_tex[1][-1], 
                select_tex2[0], select_tex2[1][0],
            )
        ))
        self.wait()
        ...
        ).arrange(DOWN).move_to(3*LEFT + 2*DOWN)
        ...
        self.play(
            Circumscribe(complexities[-1], color = RED)
        )
        self.wait()
...
class Part1(Scene):
    def construct(self):
        ...
        mult_algs_texts = [
            ...
            ["2019", "Harvey \& van Der Hoeven", "dark magic", r"$O(n\text{log}(n))$ {\tiny (bit operations)}"],
        ]


        mult_algs_group = Group(
            *[Tex(str).scale(0.5) for line in mult_algs_texts for str in line]
        ).arrange_in_grid(cols = 4).to_corner(DR)

        self.play(
            FadeIn(mult_algs_group)
        )
        self.wait()
